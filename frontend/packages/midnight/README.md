# Midnight Payload Plugin

A [Payload CMS](https://payloadcms.com) plugin that integrates Midnight blockchain wallet connectivity.

## 🌙 Midnight Integration Features

This plugin includes Midnight blockchain wallet connector components:

- **MidnightWalletConnector** - Wallet connection component using window.midnight.mnlace.enable()
- **MidnightDashboard** - Dashboard integration showing the wallet connector
- **useMidnightWallet** - React hook for wallet state management
- **Types & Interfaces** - Complete TypeScript support

### Quick Usage

```tsx
import { MidnightWalletConnector, useMidnightWallet } from 'midnight/client'

// Direct component usage
<MidnightWalletConnector 
  walletName="mnlace" 
  onConnect={(api) => console.log('Connected:', api)}
/>

// Or use the hook
const { isConnected, walletApi } = useMidnightWallet('mnlace')
```
...
Below is the original Payload plugin template documentation:
